# Layer 6: observability/ — Watching What Happens Inside the AI System
...
This layer is about visibility.

In a simple AI demo, when something goes wrong, we usually just see a bad answer. But in production, that is not enough. We need to know why the bad answer happened.
...
That is why we need the `observability/` layer.

The purpose of this layer is simple:

The `observability/` layer helps us see, trace, measure, and improve every important step inside the AI system.

### tracer.py

This file tracks the full journey of a user request. For example, when a user asks a question, the system may pass through many stages:
...
This file connects user feedback with system behavior. For example, users may click:
...
This file tracks how much each AI request costs. This is very important in production. In a demo, cost may not feel like a big issue. But when real users start using your app, every model call, embedding call, reranking call, and tool call adds cost. The `cost_tracker.py` file can track:
...
This helps you avoid surprise bills. For example, maybe one feature looks simple but internally calls the model five times. Without cost tracking, you may not notice it. With cost tracking, you can optimize it.
